...
export default function App() {
  ...
  const authenticatePi = async () => {
    ...
    try {
      ...
      let authenticatedUser = authResult.user;

      // Send returned access token to the backend for verification if available
      try {
        ...
        if (verifyResponse.ok && contentType && contentType.includes("application/json")) {
        ...
        console.warn("Backend API verification bypassed/failed:", backendError);
      }

      if (authenticatedUser) {
        setPiUser(authenticatedUser);
        localStorage.setItem("pi_user", JSON.stringify(authenticatedUser));
      } else {
        throw new Error("لم نتمكن من الحصول على بيانات المستخدم من شبكة Pi Network.");
      }
    } catch (err: any) {
